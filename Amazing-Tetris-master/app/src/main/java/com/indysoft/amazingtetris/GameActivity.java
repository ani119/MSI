...
public class GameActivity extends Activity implements GestureDetector.OnGestureListener, GestureDetector.OnDoubleTapListener {
    ...
    final int MAX_PULSE_HAPPY=140;
    final int MIN_PULSE_ANGRY=180;
    ...
    void PaintMatrix() {
        ...
        TextView scoreTextView = (TextView) findViewById(R.id.game_score_textview);
        scoreTextView.setText("Score: " + score);

        // Update the score textview
        TextView pulseTextView = (TextView) findViewById(R.id.game_pulse_textview);
        pulseTextView.setText("Pulse: " + pulse);

        //Update the face img
        SetImg();

        //Check out pulse and update speed
        UpdateSpeed();
    }
    ...
    private void SetImg()
    {
        ...
        if(pulse>MIN_PULSE_ANGRY)
        {
            face.setImageResource(R.drawable.ic_face_angry);
        }
        else if (pulse <MAX_PULSE_HAPPY)
        {
            face.setImageResource(R.drawable.ic_face_happy);
        }
        else
            face.setImageResource(R.drawable.ic_face_neutral);
